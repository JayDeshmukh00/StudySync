...
export const Dashboard = ({ plans, onSelectPlan, onCreateNew, onAnalytics, onFlashcards, onDeletePlan, onBack }) => {
    ...
    const confirmDelete = async () => {
        ...
        await fetch(`http://localhost:3001/api/plan/${planToDelete._id}`, {
            method: 'DELETE',
            headers: { 'x-auth-token': localStorage.getItem('token') }
        });
        ...
        <div className="animate-fade-in">
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
                ...
                <div className="flex items-center space-x-4">
                    <button onClick={onFlashcards} className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-4 rounded-lg transition duration-300 flex items-center">
                        <Icon path="M16.5 8.25V6a2.25 2.25 0 0 0-2.25-2.25H6A2.25 2.25 0 0 0 3.75 6v8.25A2.25 2.25 0 0 0 6 16.5h2.25m8.25-8.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-7.5A2.25 2.25 0 0 1 8.25 18v-1.5m8.25-8.25h-6a2.25 2.25 0 0 0-2.25 2.25v6" className="w-5 h-5 mr-2"/>Flashcards
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                 <div className="bg-white/10 dark:bg-black/50 backdrop-blur-sm p-6 rounded-lg border border-blue-800/30">
                    <h3 className="font-bold text-lg text-gray-800 dark:text-white mb-2">My Topic Calendar</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Feature coming soon: A visual calendar of your study topics!</p>
                </div>
                <div className="bg-white/10 dark:bg-black/50 backdrop-blur-sm p-6 rounded-lg border border-blue-800/30">
                    ...
                    {plans.map(plan => (
                        <div key={plan._id} onClick={() => onSelectPlan(plan)} className="bg-white/10 dark:bg-black/50 backdrop-blur-sm p-6 rounded-lg border border-blue-800/30 cursor-pointer transition-all duration-300 hover:border-blue-500 hover:shadow-2xl hover:shadow-blue-900/30 hover:-translate-y-1 group relative">
                            ...
                            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full mt-4 h-2.5">
                                <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${(plan.sections.filter(s => s.status === 'completed').length / plan.sections.length) * 100}%` }}></div>
                            </div>
