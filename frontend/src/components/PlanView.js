...
export const PlanView = ({ plan, setPlan, onBack, onStartAssessment, initialSectionId }) => {
    const [selectedSection, setSelectedSection] = useState(() => {
        if (initialSectionId) {
            return plan.sections.find(s => s._id === initialSectionId) || plan.sections[0];
        }
    ...
    const handleUpdateSection = useCallback(async (updatedSectionData) => {
        ...
        const originalPlan = JSON.parse(JSON.stringify(plan)); // Deep copy for potential revert
        ...
        try {
            ...
            setPlan(originalPlan); // Revert to original plan on failure
        }
        ...
        <div className="animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <section className="w-full md:w-2/3 lg:w-3/4">
                    <DayDetailView key={selectedSection._id} section={selectedSection} onUpdate={handleUpdateSection} onStartAssessment={onStartAssessment} planId={plan._id} />
                </section>
            </div>
        </div>
    );
